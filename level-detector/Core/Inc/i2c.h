...
/**
 * @brief Specifies an approximation of the maximum value of gravity read by the IMU at rest.
 *
 * It is used as a "maximum range" value that can be expected from a single axis
 * of data while the IMU is at rest. This value may be exceeded while the device
 * is in motion; the excess value will be truncated to MAX_AT_REST because it does
 * not provide any meaningful information for the device's use case.
 */
static const int16_t MAX_AT_REST = 17000;
